...
// AdmissionWebhookPatch scaffolds a AdmissionWebhookPatch for a Resource
type AdmissionWebhookPatch struct {
	input.Input

	// Resource is the Resource to make the AdmissionWebhookPatch for
...
func (p *AdmissionWebhookPatch) GetInput() (input.Input, error) {
	if p.Path == "" {
		p.Path = filepath.Join("config", "crds", "patches",
			fmt.Sprintf("webhook_in_%s.yaml", strings.ToLower(p.Resource.Kind)))
	}
	p.TemplateBody = AdmissionWebhookPatchTemplate
	return p.Input, nil
}

// Validate validates the values
func (g *AdmissionWebhookPatch) Validate() error {
